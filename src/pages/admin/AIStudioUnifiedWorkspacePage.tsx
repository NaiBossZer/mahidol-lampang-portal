...
const modules: Array<{ id: ModuleId; label: string; description: string; icon: typeof Bot }> = [
  { id: "workflow", label: "9-Step Workflow", description: "Activity → Outcome → Publish", icon: Workflow },
  { id: "documents", label: "คลังเอกสารราชการ", description: "Official Documents Repository", icon: FileText },
  { id: "assistant", label: "AI Assistant Studio", description: "Command · Queue · Execution · Approval", icon: Bot },
  { id: "access", label: "ผู้ใช้งาน & สิทธิ์", description: "Users · Organizations · Access", icon: Group },
  { id: "notifications", label: "การแจ้งเตือน", description: "Governance · Notifications", icon: Bell },
  { id: "settings", label: "ตั้งค่าระบบ", description: "System Configuration", icon: Settings },
];
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-white/5 p-3 sm:p-4">
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
        <div><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-sky-200">Unified Workflow</p><p className="text-xs font-semibold text-white">Phase 1 · Activity & AI Survey <span className="mx-1 text-white/30">→</span> Phase 2 · Outcome & Publication</p></div>
...
export function AIStudioUnifiedWorkspacePage() {
  ...
  return <section className="min-h-[calc(100vh-4rem)] bg-[#f8f9ff] px-4 py-5 sm:px-6 lg:px-8"><div className="mx-auto max-w-[1400px] space-y-5">
    ...
    <ModuleCard active={module} onSelect={setModule} />

    <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
      {module === "workflow" && <div className="border-b border-slate-100 bg-slate-50 px-5 py-3 sm:px-6"><p className="text-xs font-bold text-[#002d62]">Step {activeStep} / 9 · {steps[activeStep - 1].title}</p><p className="mt-0.5 text-[10px] text-slate-500">{steps[activeStep - 1].subtitle} · AI proposes · ADMIN verifies · System commits</p></div>}
      <div className={module === "workflow" ? "p-0" : "p-0"}>
        {module === "workflow" && activeStep <= 5 && <AIStudioWorkspacePage />}
        {module === "workflow" && activeStep >= 6 && <AIStudioOutcomeWorkspacePage />}
        {module === "documents" && <CmsPage />}
        {module === "assistant" && <AIWorkspacePage />}
        {module === "access" && <OrganizationsManagementPage />}
        {module === "notifications" && <GovernancePage />}
        {module === "settings" && <SystemSettingsPage />}
      </div>
    </div>
