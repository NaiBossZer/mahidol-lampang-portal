...
export function AIStudioWorkspacePage() {
  ...
  async function handleConfirmSurvey() {
    ...
    setConfirming(true);
    try {
      const result = await confirmAiSurvey(
        surveyExecutionId,
        "approved",
        undefined,
        generatedSurvey,
              ...
              <StudioCard eyebrow="03 · GENERATION" title="AI Survey Composer" icon={<Sparkles className="h-4 w-4" />}>
                <div className="space-y-5">
                  ...
                  {!generatedSurvey ? <div className="rounded-xl border border-dashed border-slate-200 p-8 text-center text-xs text-slate-400">ต้องมีผล AI Analysis ก่อนจึงจะสร้างแบบประเมินได้</div> : <>
                    <div className="grid gap-3 sm:grid-cols-3"><label className="rounded-xl bg-slate-50 p-3"><span className="text-[10px] text-slate-400">ชื่อแบบประเมิน</span><input value={generatedSurvey.surveyTitle} onChange={(event) => setGeneratedSurvey((current) => current ? { ...current, surveyTitle: event.target.value } : current)} className="mt-1 h-9 w-full rounded-lg border border-slate-200 bg-white px-2 text-xs font-bold text-slate-700" /></label><div className="rounded-xl bg-slate-50 p-3"><p className="text-[10px] text-slate-400">Scale</p><p className="mt-1 text-xs font-bold text-slate-700">{generatedSurvey.scaleType}</p></div><div className="rounded-xl bg-slate-50 p-3"><p className="text-[10px] text-slate-400">AI confidence</p><p className="mt-1 text-xs font-bold text-emerald-600">{Math.round(generatedSurvey.aiConfidenceScore * 100)}%</p></div></div>
