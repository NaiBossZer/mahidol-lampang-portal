...
function textAreaField(key: string) {
  return ["description", "summary", "subtitle", "address", "objective", "copyrightText"].includes(
    key,
  );
}
  ...
  async function save() {
    ...
    try {
      ...
      const response = await fetch(
        isNew ? endpoint : `${endpoint}?id=${encodeURIComponent(String(draft.id))}`,
        {
      ...
      if (mediaFile && mediaConfig && saved.id) {
        const media = await uploadMedia(
          ...
          String(saved.id),
          mediaFile,
          String(saved.title ?? saved.name ?? saved.label ?? mediaLabel(tab)),
        );
        ...
        const updateResponse = await fetch(
          `${endpoint}?id=${encodeURIComponent(String(saved.id))}`,
          {
            method: "PUT",
            credentials: "include",
            headers: { "Content-Type": "application/json", Accept: "application/json" },
        ...
        if (!updateResponse.ok || !updateBody.data)
          throw new Error(updateBody.error ?? "บันทึก URL ของ Storage ไม่สำเร็จ");
      ...
      setMediaFile(null);
      toast.success(
        mediaFile && mediaConfig ? "บันทึกเนื้อหาและไฟล์ใน Storage แล้ว" : "บันทึกเนื้อหาแล้ว",
      );
      void saved;
      await load();
    } catch (error) {
    ...
    <section className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <h1 className="mt-1 text-3xl font-black text-brand-navy">Central Content Management</h1>
          <p className="mt-2 max-w-3xl text-sm text-slate-600">
            ข้อมูลเนื้อหาอยู่ใน Central Portal และไฟล์ทุกฟิลด์จะถูกเก็บผ่าน Supabase Storage
          </p>
        </div>
        <div className="flex gap-2">
          <button
            type="button"
            onClick={() => void load()}
            className="inline-flex min-h-11 items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 text-sm font-bold text-brand-navy"
          >
      ...
      <div className="mt-6 flex gap-2 overflow-x-auto border-b border-slate-200 pb-2">
        {tabs.map((item) => (
          <button
            ...
            className={`shrink-0 rounded-lg px-3 py-2 text-sm font-bold ${tab === item.id ? "bg-brand-navy text-white" : "text-slate-600 hover:bg-slate-100"}`}
          >
            {item.label}
          </button>
        ))}
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_420px]">
        <div className="rounded-2xl border border-slate-200 bg-white shadow-sm">
          <div className="border-b border-slate-100 px-5 py-4 font-bold text-brand-navy">
            {tabs.find((x) => x.id === tab)?.label}
          </div>
          {loading ? (
            <p className="p-5 text-sm text-slate-500">กำลังโหลด...</p>
          ) : rows.length === 0 ? (
            <p className="p-5 text-sm text-slate-500">ยังไม่มีข้อมูล</p>
          ) : (
              ...
              {rows.map((row, index) => (
                ...
                >
                  <p className="font-bold text-slate-900">
                    {String(row.title ?? row.name ?? row.label ?? row.sectionKey ?? "รายการ")}
                  </p>
                  <p className="mt-1 text-xs text-slate-500">
                    {String(row.slug ?? row.status ?? row.targetUrl ?? "")}{" "}
                    {row.sortOrder !== undefined ? `· ลำดับ ${String(row.sortOrder)}` : ""}
                  </p>
                  <p className="mt-2 line-clamp-2 text-sm text-slate-600">
                    {String(row.summary ?? row.description ?? row.subtitle ?? "")}
                  </p>
                </button>
              ))}
            </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="flex items-center justify-between gap-3">
            <h2 className="font-bold text-brand-navy">
              {draft ? (draft.id ? "แก้ไขเนื้อหา" : "สร้างเนื้อหา") : "เลือกเนื้อหา"}
            </h2>
            {draft && (
              <button
                type="button"
          ...
          {!draft ? (
            <p className="mt-3 text-sm text-slate-500">เลือกข้อมูลจากรายการ หรือสร้างรายการใหม่</p>
          ) : (
            <div className="mt-4 space-y-4">
              ...
              {mediaConfig && (
                <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-4">
                  <div className="flex items-start gap-3">
                    <div className="grid h-10 w-10 place-items-center rounded-xl bg-white text-brand-navy">
                      <FileImage className="h-5 w-5" />
                    </div>
                    <div>
                      <p className="text-sm font-bold text-brand-navy">{mediaConfig.label}</p>
                      <p className="mt-1 text-xs text-slate-500">
                        เพิ่มไฟล์ผ่าน Supabase Storage · bucket:{" "}
                        <span className="font-semibold">portal-media</span> · JPEG / PNG / WebP /
                        PDF · ไม่เกิน 25MB
                      </p>
                    </div>
                  </div>
                  {String(draft[mediaConfig.key] ?? "") && (
                    <div className="mt-3 rounded-lg bg-white p-2 text-xs text-slate-500 break-all">
                      ไฟล์ปัจจุบัน: {String(draft[mediaConfig.key])}
                    </div>
                  )}
                  <label className="mt-3 flex cursor-pointer items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-bold text-brand-navy hover:bg-slate-50">
                    <FileUp className="h-4 w-4" />
                    {mediaFile ? mediaFile.name : `เลือก ${mediaConfig.label}`}
                    <input
                      type="file"
                  ...
                  {mediaFile && (
                    <p className="mt-2 text-xs text-emerald-700">
                      พร้อมอัปโหลด: {mediaFile.name} ({Math.ceil(mediaFile.size / 1024)} KB)
                    </p>
                  )}
                </div>
              )}
              ...
              <div className="flex gap-2 pt-2">
                <button
                  ...
                  disabled={saving}
                  onClick={() => void save()}
                  className="inline-flex min-h-11 flex-1 items-center justify-center gap-2 rounded-xl bg-brand-navy px-4 font-bold text-white disabled:opacity-60"
                >
                ...
                <button
                  ...
                  className="min-h-11 rounded-xl border border-slate-200 px-4 font-bold"
                >
                  ยกเลิก
                </button>
              </div>
