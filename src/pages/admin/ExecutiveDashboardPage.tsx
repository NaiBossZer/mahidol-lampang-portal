...
export function ExecutiveDashboardPage() {
  ...
  const questionScores = SCORE_FIELDS.map((field, index) => ({ field, label: LABELS[index], value: average(responses.map((response) => score(response[field])).filter((value): value is number => value !== null)) })).filter((item) => item.value !== null) as { field: (typeof SCORE_FIELDS)[number]; label: string; value: number }[];
