...
export function ExecutiveDashboardPage() {
  ...
  return <div className="min-h-[calc(100vh-4rem)] bg-slate-50/70 text-slate-900"><main className="mx-auto max-w-[1440px] px-4 py-6 sm:px-6 xl:px-8">
    ...
    <section className="mb-5 grid gap-5 lg:grid-cols-[1.1fr_0.9fr_1.1fr]"><Panel title="ความพึงพอใจรายด้าน" subtitle="AVERAGE SCORE BY TOPIC">{topicScores.map((t) => <div key={t.key} className="mb-5 last:mb-0"><div className="flex justify-between text-xs font-semibold"><span>{t.label}</span><span>{t.avg === null ? "—" : `${t.avg.toFixed(2)} / 5.00`}</span></div><div className="mt-2 h-2.5 overflow-hidden rounded-full bg-slate-100"><div className={`h-full rounded-full ${t.className}`} style={{ width: `${t.percent ?? 0}%` }}/></div></div>)}</Panel><Panel title="ผู้ตอบแบบประเมิน" subtitle="RESPONSE SUMMARY"><div className="grid place-items-center py-3"><div className="grid h-36 w-36 place-items-center rounded-full border-[14px] border-brand-blue/15"><div className="text-center"><div className="font-numeric text-3xl font-bold text-brand-navy">{responses.length.toLocaleString()}</div><div className="text-[10px] text-slate-500">ผู้ตอบ</div></div></div></div><div className="rounded-xl bg-slate-50 p-3 text-xs"><div className="flex justify-between"><span>ผู้เข้าร่วม</span><b>{participants.toLocaleString()} คน</b></div><div className="mt-2 flex justify-between"><span>ผู้ตอบแบบประเมิน</span><b>{responses.length.toLocaleString()} คน</b></div><div className="mt-2 flex justify-between"><span>Response Rate</span><b>{responseRate === null ? "—" : `${responseRate.toFixed(1)}%`}</b></div></div></Panel><Panel title="ภาพกิจกรรม" subtitle="ACTIVITY DOCUMENTATION" badge={`${photos.length} ภาพ`}>{photos.length ? <div className="grid grid-cols-2 gap-2.5">{photos.map((p) => <figure key={p.id} className="overflow-hidden rounded-xl border border-slate-200"><img src={p.image} alt={p.title} className="aspect-[4/3] w-full object-cover"/><figcaption className="truncate px-2.5 py-2 text-xs font-semibold text-slate-700">{p.title}</figcaption></figure>)}</div> : <Empty text="ยังไม่มีภาพกิจกรรม"/>}</Panel></section>
