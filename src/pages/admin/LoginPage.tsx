    ...
    <div className="h-dvh overflow-hidden bg-surface-warm px-4 py-4 font-sans sm:px-6 sm:py-5">
      <main className="mx-auto flex h-full w-full max-w-[520px] flex-col justify-center">
        <header className="shrink-0 text-center">
          <img src="/social-engagement-logo.png" alt="งานพันธกิจเพื่อสังคม" className="mx-auto h-16 w-16 object-contain sm:h-20 sm:w-20" width="80" height="80" />
        ...
        <section className="mt-4 shrink-0 rounded-2xl border border-panel-line bg-white px-5 py-5 shadow-[0_10px_30px_rgba(18,59,99,0.08)] sm:px-7 sm:py-6">
          ...
          <form onSubmit={handleLogin} className="mt-5 space-y-4" autoComplete="on">
            <div className="space-y-1.5">
              ...
              <input id="login-email" type="email" required value={email} onChange={(e) => { setEmail(e.target.value); setError(""); }} placeholder="username@mahidol.ac.th" className="w-full rounded-xl border border-input bg-white px-4 py-2.5 text-sm text-ink outline-none transition placeholder:text-muted-ink/70 focus:border-brand-blue focus:ring-2 focus:ring-brand-blue/15" autoFocus autoComplete="username" />
            </div>
            ...
            <button type="submit" disabled={loading} className="w-full rounded-xl bg-brand-navy py-2.5 text-sm font-bold text-white shadow-[0_5px_14px_rgba(18,59,99,0.18)] transition hover:bg-[#0F3553] active:scale-[0.99] disabled:cursor-not-allowed disabled:opacity-60">{loading ? "กำลังตรวจสอบ…" : "เข้าสู่ระบบ"}</button>
          </form>
