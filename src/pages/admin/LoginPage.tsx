...
export function LoginPage() {
  ...
  const redirect =
    redirectParam?.startsWith("/admin") || redirectParam === "/dashboard"
      ? redirectParam
      : "/dashboard";
      ...
      <main className="mx-auto flex h-full w-full max-w-[520px] flex-col justify-center">
        <header className="shrink-0 text-center">
          <img
            ...
            height="80"
          />
          <h1 className="mt-2 text-lg font-black tracking-tight text-brand-navy sm:text-xl">
            งานพันธกิจเพื่อสังคม
          </h1>
          <p className="mt-0.5 text-[11px] font-medium leading-4 text-muted-ink sm:text-xs">
        ...
        <section className="mt-4 shrink-0 rounded-2xl border border-panel-line bg-white px-5 py-5 shadow-[0_10px_30px_rgba(18,59,99,0.08)] sm:px-7 sm:py-6">
          ...
          <form onSubmit={handleLogin} className="mt-5 space-y-4" autoComplete="on">
            ...
            <div className="space-y-1.5">
              <div className="flex items-center justify-between gap-3">
                ...
                <span className="text-[11px] font-semibold text-brand-blue sm:text-xs">
            ...
            {error && (
              <p
                role="alert"
                className="rounded-xl border border-red-100 bg-red-50 px-3 py-2 text-center text-xs font-semibold leading-5 text-red-600"
              >
                {error}
              </p>
            )}
          ...
          <div className="mt-4 border-t border-surface-subtle pt-4 text-center">
            <RouterLink
              to="/"
              className="inline-flex min-h-10 items-center rounded-lg px-3 py-1.5 text-sm font-semibold text-muted-ink transition-colors hover:bg-surface-subtle hover:text-brand-blue"
            >
              ← กลับสู่หน้าหลัก
            </RouterLink>
          </div>
