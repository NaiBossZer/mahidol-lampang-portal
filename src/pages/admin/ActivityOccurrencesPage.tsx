    ...
    <section className="mx-auto max-w-6xl px-4 py-6 sm:px-6 lg:px-8">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h1 className="mt-1 text-2xl font-bold text-brand-navy lg:text-3xl">รอบการจัดกิจกรรม</h1>
          <p className="mt-1 text-sm text-slate-600">
            จัดการการจัดกิจกรรมแต่ละครั้ง โดยไม่สร้าง Activity ซ้ำ
          </p>
        </div>
        <div className="flex gap-2">
          ...
          <button
            ...
            onClick={add}
            disabled={!activityId}
            className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-brand-navy px-4 text-sm font-bold text-white disabled:opacity-50"
          >
      ...
      <div className="mt-4 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        {loading ? (
          ...
          <div className="p-10 text-center text-sm text-slate-500">
            ยังไม่มีรอบการจัดกิจกรรมสำหรับ {selectedName}
          </div>
        ) : (
                ...
                >
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs font-semibold text-emerald-700">{x.status}</span>
                    </div>
                    ...
                    <p className="mt-1 text-sm text-slate-500">
                      {x.location_detail || "ไม่ระบุสถานที่"} · ผู้เข้าร่วม {x.participant_count}
                    </p>
                  </div>
                  <button
                    type="button"
                    onClick={() => void archive(x.id)}
                    className="inline-flex min-h-10 items-center justify-center gap-2 rounded-xl border border-slate-200 px-3 text-sm font-semibold text-slate-600"
                  >
        ...
        <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/40 p-4">
          <div className="w-full max-w-2xl rounded-2xl bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-200 p-5">
              <h2 className="text-lg font-bold text-brand-navy">เพิ่มรอบการจัดกิจกรรม</h2>
            ...
            <div className="flex justify-end gap-2 border-t border-slate-200 p-5">
              ...
              <button
                type="button"
                onClick={() => void save()}
                className="min-h-11 rounded-xl bg-brand-navy px-5 text-sm font-bold text-white"
              >
                บันทึก
              </button>
            </div>
