...
function resolveTab(value: string | null): TabId {
  return TABS.some((tab) => tab.id === value) ? (value as TabId) : "studio";
}
...
export function AIWorkspacePage() {
  ...
  function setTab(tab: TabId) {
    navigate(tab === "studio" ? "/admin/ai" : `/admin/ai?tab=${tab}`);
  }
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[.14em] text-emerald-700">
                <Bot className="h-3.5 w-3.5" /> AI WORKSPACE
              </div>
              <h1 className="mt-1 text-2xl font-bold tracking-tight text-[#002d62] sm:text-[30px]">ศูนย์ทำงาน AI</h1>
              <p className="mt-1 max-w-2xl text-sm text-slate-600">รวม UX/UI workflow จาก AI Studio และศูนย์ควบคุม AI Production ไว้ใน Workspace เดียว โดยยังคง Governance, RBAC และ Audit เดิม</p>
            </div>
            <div className="flex items-center gap-2 rounded-xl border bg-slate-50 px-3 py-2">
              <CheckCircle2 className="h-4 w-4 text-emerald-700" />
              <div><p className="text-[10px] font-bold uppercase tracking-wide text-slate-400">Current view</p><p className="text-xs font-semibold text-slate-700">{activeMeta.label} · {activeMeta.description}</p></div>
            </div>
          </div>
          <div className="mt-5 overflow-x-auto">
            <div className="flex min-w-max gap-2 pb-3" role="tablist" aria-label="AI Workspace">
              {TABS.map((tab) => {
                ...
                return <button key={tab.id} type="button" role="tab" aria-selected={active} onClick={() => setTab(tab.id)} className={active ? "inline-flex items-center gap-2 rounded-xl bg-[#002d62] px-4 py-2.5 text-sm font-bold text-white shadow-sm" : "inline-flex items-center gap-2 rounded-xl border bg-white px-4 py-2.5 text-sm font-semibold text-slate-600 hover:border-[#002d62]/30 hover:text-[#002d62]"}><Icon className="h-4 w-4" />{tab.label}</button>;
              })}
            </div>
          </div>
        </div>
