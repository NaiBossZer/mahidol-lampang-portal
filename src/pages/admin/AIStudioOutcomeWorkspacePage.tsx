...
import { ArrowLeft, Check, CheckCircle2, ImagePlus, Loader2, Newspaper, Rocket, Sparkles, Upload } from "lucide-react";
...
async function uploadMedia(activityId: string, file: File): Promise<ActivityMedia> {
  ...
  form.set("activityId", activityId); form.set("file", file); form.set("isPostEvent", "true");
...
export function AIStudioOutcomeWorkspacePage() {
  ...
  const outcomeReady = participantCount > 0 || Boolean(form.outcome || form.impact || media.length);
  ...
  useEffect(() => {
    ...
    setReport(null); setReview({ dataVerified: false, narrativeReviewed: false, mediaSelected: false, pdpaChecked: false });
  ...
  async function captureFiles() {
    ...
    try {
      ...
      setMedia((current) => [...uploaded, ...current]); setFiles([]);
