...
export function ActivityMediaPage() {
  ...
  async function loadMedia(id: string) {
    ...
    const r = await fetch(`/api/admin/activity-media?activityId=${encodeURIComponent(id)}`, {
      credentials: "include",
      headers: { Accept: "application/json" },
    });
    const b = await r.json();
    if (!r.ok) throw new Error(b?.error || "โหลดภาพไม่สำเร็จ");
    setMedia(b.data ?? []);
  }
  async function load() {
    setLoading(true);
  ...
  useEffect(() => {
    void loadMedia(activityId);
  }, [activityId]);
  async function upload() {
    if (!activityId || !file) return toast.error("เลือกกิจกรรมและไฟล์ภาพก่อน");
    const f = new FormData();
    f.set("activityId", activityId);
    f.set("file", file);
    try {
      const r = await fetch("/api/admin/activity-media", {
        method: "POST",
        credentials: "include",
        body: f,
      });
      const b = await r.json();
      if (!r.ok) throw new Error(b?.error || "อัปโหลดไม่สำเร็จ");
      setFile(null);
      ...
      <div className="flex items-end justify-between">
        <div>
          ...
          <h1 className="mt-1 text-2xl font-bold text-brand-navy lg:text-3xl">
            ภาพกิจกรรมหลังเสร็จสิ้นกิจกรรม
          </h1>
        </div>
        <button
          type="button"
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {media.map((m) => (
            <figure key={m.id} className="overflow-hidden rounded-2xl border bg-white shadow-sm">
              <img
                src={m.public_url}
                alt={m.caption ?? "ภาพกิจกรรม"}
                className="aspect-[4/3] w-full object-cover"
              />
              <figcaption className="p-3 text-xs text-slate-500">
                {m.caption ?? m.storage_path}
              </figcaption>
            </figure>
          ))}
          {media.length === 0 && (
            <p className="text-sm text-slate-500">ยังไม่มีภาพสำหรับกิจกรรมนี้</p>
          )}
        </div>
      )}
    </section>
