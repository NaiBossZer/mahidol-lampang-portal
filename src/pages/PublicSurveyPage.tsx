...
export function PublicSurveyPage() {
  ...
  useEffect(() => {
    fetch("/api/survey").then(async (r) => { const body = await r.json(); if (!r.ok) throw new Error(body.error || "ไม่สามารถโหลดรายการแบบสอบถามได้"); return body.data as SurveySummary[]; }).then((rows) => { setAvailable(rows); if (querySurveyId) void loadSurvey(querySurveyId); else setLoading(false); }).catch((e) => { setError(e instanceof Error ? e.message : "ไม่สามารถโหลดรายการแบบสอบถามได้"); setLoading(false); });
  }, [querySurveyId]);
