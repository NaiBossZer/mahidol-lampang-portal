...
export function HomePage() {
  ...
  useEffect(() => {
    getActivities().then((items) => setActivities(items.slice(0, 6)));
  }, []);
  return (
    <div className="min-h-screen bg-[#f8f6f0] text-slate-800">
      <header className="sticky top-0 z-50 border-b border-white/10 bg-[#0e2b42]/95 text-white backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-3 sm:px-6 lg:px-8">
          ...
          <nav className="flex items-center gap-2 text-xs font-bold">
            <RouterLink to="/activities" className="rounded-full px-3 py-2 hover:bg-white/10">
              กิจกรรม
            </RouterLink>
            <RouterLink to="/projects" className="rounded-full px-3 py-2 hover:bg-white/10">
              โครงการ
            </RouterLink>
            <RouterLink to="/centers" className="rounded-full px-3 py-2 hover:bg-white/10">
              ศูนย์ / พื้นที่
            </RouterLink>
          </nav>
        </div>
      ...
      <section className="relative overflow-hidden bg-[#123B63] text-white">
        <div className="absolute inset-0 opacity-30">
          <img src="/main banner.jpg" alt="" className="h-full w-full object-cover" />
        </div>
        <div className="relative mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8 lg:py-24">
          <div className="max-w-3xl">
            <span className="rounded-full bg-[#D6A84F] px-3 py-1 text-xs font-black text-[#123B63]">
            ...
            <p className="mt-5 max-w-2xl text-sm leading-7 text-blue-100 sm:text-base">
              แพลตฟอร์มกลางสำหรับกิจกรรม โครงการ ศูนย์ปฏิบัติการ
              และระบบการเรียนรู้ของคณะสิ่งแวดล้อมและทรัพยากรศาสตร์ มหาวิทยาลัยมหิดล ในพื้นที่สบปราบ
              ลำปาง
            </p>
            <div className="mt-7 flex flex-wrap gap-3">
              <RouterLink
                to="/activities"
                className="rounded-xl bg-[#D6A84F] px-5 py-3 text-sm font-black text-[#123B63]"
              >
                ดูผลงานและกิจกรรม <ArrowUpRight className="ml-1 inline h-4 w-4" />
              </RouterLink>
              <RouterLink
                to="/centers"
                className="rounded-xl border border-white/30 px-5 py-3 text-sm font-bold hover:bg-white/10"
              >
        ...
        <div className="flex items-end justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              แต่ละระบบมี Learning Experience เป็นส่วนหนึ่งของระบบนั้น
            </p>
          </div>
        </div>
            ...
            >
              ...
              <div className="p-6">
                ...
                <div className="mt-5 flex items-center justify-between">
                  <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-bold text-[#123B63]">
                    Learning Experience
                  </span>
                  <a
                    href={system.href}
                    ...
                    rel="noreferrer"
                    className="text-sm font-bold text-[#1677A8]"
                  >
                    เข้าสู่ระบบ <ExternalLink className="inline h-4 w-4" />
                  </a>
                </div>
              </div>
            </article>
      ...
      <section className="bg-white py-12">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex items-end justify-between">
            <div>
              <p className="text-xs font-black uppercase tracking-widest text-[#C66B4F]">
                ACTIVITY EVIDENCE
              </p>
              <h2 className="mt-2 text-2xl font-black text-[#123B63]">กิจกรรมล่าสุด</h2>
            </div>
            <RouterLink to="/activities" className="text-sm font-bold text-[#1677A8]">
              ดูทั้งหมด →
            </RouterLink>
          </div>
        ...
        <div className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
          <div className="rounded-3xl bg-[#123B63] p-7 text-white">
            ...
            <p className="mt-3 text-sm leading-7 text-blue-100">
              ทุกกิจกรรมสามารถบันทึกวันเวลา พื้นที่ ผู้เข้าร่วม ภาคีเครือข่าย ผลลัพธ์
              และหลักฐานภาพถ่าย เพื่อรองรับการสรุปผลระดับผู้บริหารในระยะถัดไป
            </p>
            <RouterLink
              to="/activities"
              className="mt-5 inline-flex items-center gap-2 rounded-xl bg-[#D6A84F] px-4 py-2.5 text-sm font-black text-[#123B63]"
            >
              สำรวจ Activity Evidence <BarChart3 className="h-4 w-4" />
            </RouterLink>
          </div>
      ...
      <footer className="bg-[#0e2b42] py-8 text-white">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <p className="mt-4 text-[11px] text-slate-400">
            © 2026 Mahidol Social Engagement Platform
          </p>
        </div>
      </footer>
    </div>
...
function ActivityPreview({ activity }: { activity: SocialActivity }) {
  return (
    <RouterLink
      ...
      className="group overflow-hidden rounded-3xl border border-slate-200 bg-[#f8f6f0] transition hover:shadow-lg"
    >
      <img
      ...
      />
      <div className="p-5">
        <div className="flex items-center gap-2 text-[11px] font-bold text-[#C66B4F]">
