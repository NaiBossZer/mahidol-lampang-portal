...
export function ActivityDetailPage() {
  ...
  if (!activity)
    return (
      <PublicAppShell>
        <main
          className="flex min-h-[60vh] items-center justify-center bg-surface-warm px-4 text-slate-500"
          role="status"
          aria-live="polite"
        >
          ไม่พบข้อมูลกิจกรรม
      ...
      <div className="bg-surface-warm text-slate-800">
        <section className="bg-brand-navy text-white" aria-labelledby="activity-detail-title">
          <div className="mx-auto max-w-6xl px-4 py-6 sm:px-6 lg:px-8">
            ...
            <p className="mt-5 text-xs font-bold uppercase tracking-[0.18em] text-northern-gold">
              พันธกิจเพื่อสังคม
            </p>
            <h1
              id="activity-detail-title"
              className="mt-2 max-w-4xl text-3xl font-black sm:text-5xl"
            >
              {activity.title}
            </h1>
        ...
        <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8 lg:py-12">
          ...
          <div className="mt-8 grid gap-6 lg:grid-cols-[1.3fr_0.7fr]">
            ...
            <aside
              ...
              aria-labelledby="activity-evidence-title"
            >
              <p className="text-xs font-bold uppercase tracking-widest text-local-terracotta">
                ACTIVITY EVIDENCE
              </p>
              <h2 id="activity-evidence-title" className="mt-2 text-xl font-black text-brand-navy">
                ข้อมูลกิจกรรม
              </h2>
              <dl className="mt-5 space-y-4 text-sm">
                <div>
                  <dt className="text-xs text-slate-400">ระบบหลัก</dt>
                  <dd className="mt-1 font-bold text-brand-navy">{systemLabel(activity.system)}</dd>
                </div>
                {activity.centerName && (
                  <div>
  ...
  return (
    <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
      <div className="flex items-center gap-2 text-brand-navy">
        <Icon className="h-5 w-5 text-local-terracotta" aria-hidden="true" />
