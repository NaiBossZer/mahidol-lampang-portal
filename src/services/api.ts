...
export type DashboardLearningCenter = {
  id: string;
  name: string;
  slug: string;
  type: string;
...
export type DashboardSurveyAnswer = {
  id: string;
  response_id: string;
  question_id: string;
...
export type DashboardResponse = {
  ...
  p2_location?: number | null;
  p2_schedule?: number | null;
  p2_readiness?: number | null;
  p2_reception?: number | null;
  p2_overall?: number | null;
  p3_interest?: number | null;
  p3_content?: number | null;
  p3_clarity?: number | null;
  p3_benefit?: number | null;
  p3_application?: number | null;
  p4_knowledge?: number | null;
  p4_inspiration?: number | null;
...
export type DashboardActivityOrganizer = {
  ...
  organizer_role: "primary" | "co";
};
export type DashboardActivityMedia = {
  id: string;
  activity_id: string;
  occurrence_id?: string | null;
  public_url: string;
...
function isCacheableGet(url: string) {
  try {
...
export async function apiRequest<T>(url: string, init?: RequestInit): Promise<T> {
  ...
  if (method !== "GET" || !isCacheableGet(url)) return request<T>(url, init);
  const cached = getCache.get(url);
  if (cached && cached.expiresAt > Date.now()) {
    getCache.delete(url);
    getCache.set(url, cached);
...
function standardsFromProduction(raw: Record<string, unknown>): Product["standards"] {
  ...
  if (raw.researchTag || text.includes("วิจัย") || text.includes("research"))
    values.add("งานวิจัย");
  return [...values];
}
export async function getProducts(): Promise<Product[]> {
  ...
  return data.flatMap((raw) => {
    ...
    const base = MOCK_PRODUCTS.find((x) => x.name === raw.name) ?? MOCK_PRODUCTS[0];
    if (!base) return [];
    ...
    return [
      {
        ...
        name: raw.name,
        image:
          typeof raw.imageUrl === "string" && raw.imageUrl ? raw.imageUrl : "/mahidol-logo.png",
        standards: standardsFromProduction(raw),
        price: Number.isFinite(price) ? price : 0,
        unit: typeof raw.unit === "string" ? raw.unit : "หน่วย",
        stock: Number.isFinite(stock) ? stock : 0,
        isPreOrder: raw.isPreorder === true,
        harvestDate: typeof raw.harvestDate === "string" ? raw.harvestDate : undefined,
...
export async function createProduct(p: ProductWriteInput) {
  const result = await apiRequest("/api/products", { method: "POST", body: JSON.stringify(p) });
  invalidateApiCache("/api/products");
  return result;
}
export async function updateProduct(p: ProductWriteInput & { id: string }) {
  const result = await apiRequest(`/api/products?id=${encodeURIComponent(p.id)}`, {
    method: "PUT",
...
export async function updateInventory(id: string, stock: number) {
  const result = await apiRequest(`/api/inventory?id=${encodeURIComponent(id)}`, {
    method: "PATCH",
...
export async function uploadSlip(file: File): Promise<string> {
  ...
  const result = await apiRequest<{ path: string }>("/api/uploads/slip", {
    method: "POST",
...
export async function getAdminActivities(): Promise<AdminActivity[]> {
  const d = await apiRequest<AdminActivity[]>("/api/admin/activities");
  return Array.isArray(d) ? d : [];
}
export async function createActivity(input: ActivityWriteInput) {
  const result = await apiRequest("/api/admin/activities", {
    method: "POST",
...
export async function deleteActivity(id: string) {
  await apiRequest(`/api/admin/activities?id=${encodeURIComponent(id)}`, { method: "DELETE" });
  invalidateApiCache("/api/admin/activities");
  invalidateApiCache("/api/activities");
}
