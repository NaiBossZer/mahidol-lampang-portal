  ...
  return (
    <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
      <div className="min-w-0">
        ...
        <h1 className="mt-1 text-2xl font-bold tracking-tight text-[#002d62] lg:text-3xl">
          {title}
        </h1>
        {description && (
...
export function AdminButton({ variant = "primary", icon, className = "", children, ...props }: AdminButtonProps) {
  ...
  return (
    <button
      {...props}
      className={`inline-flex min-h-10 items-center justify-center gap-2 rounded-xl px-4 text-sm font-semibold transition-colors disabled:cursor-not-allowed disabled:opacity-50 ${variants[variant]} ${className}`}
    >
      {icon}
      {children}
    </button>
...
export function AdminCard({ className = "", children, ...props }: HTMLAttributes<HTMLDivElement>) {
  return (
    <div {...props} className={`rounded-2xl border border-slate-200 bg-white shadow-sm ${className}`}>
      {children}
    </div>
  );
}
      ...
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex min-w-0 flex-1 flex-wrap items-center gap-2">
          {search !== undefined && onSearchChange && (
            <AdminSearchInput value={search} onChange={onSearchChange} placeholder={placeholder} />
          )}
          {children}
        </div>
        {onRefresh && (
          <AdminButton variant="secondary" icon={<RefreshCw className="h-4 w-4" />} onClick={onRefresh} aria-label="รีเฟรชข้อมูล">
            รีเฟรช
          </AdminButton>
        )}
      </div>
  ...
  return (
    <label className="relative block min-w-[220px] flex-1 lg:max-w-xl">
      ...
      <input
        ...
        className="h-11 w-full rounded-xl border border-slate-200 bg-white pl-10 pr-3 text-sm outline-none transition focus:border-[#002d62] focus:ring-4 focus:ring-[#002d62]/10"
      />
    </label>
  );
}
...
export function AdminStatusBadge({ status, label, tone, children }: AdminStatusBadgeProps) {
  ...
  return (
    <span className={`inline-flex rounded-full border px-2.5 py-1 text-xs font-semibold ${tones[resolvedTone]}`}>
