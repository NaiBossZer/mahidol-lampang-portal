          ...
          <div className="flex flex-wrap items-center gap-2">
            <div className="rounded-xl border border-white/15 bg-white/5 px-4 py-2.5">
              <div className="flex items-center gap-2 text-xs font-semibold">
                <ShieldCheck className="h-4 w-4 text-[#95f8a7]" /> Governed Activity Data
              </div>
  ...
  return (
    <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
      <div className="min-w-0">
        {eyebrow && (
          <p className="text-xs font-semibold uppercase tracking-[0.14em] text-violet-600">{eyebrow}</p>
        )}
...
function renderAdminTableChild(child: ReactNode, index: number) {
  ...
  if (childType === AdminLoading || childType === AdminLoadingState) {
    return <tbody key={`loading-${index}`}><tr><td colSpan={100} className="p-0">{child}</td></tr></tbody>;
  }
  if (childType === AdminEmptyState || childType === AdminEmpty) {
    return <tbody key={`empty-${index}`}><tr><td colSpan={100} className="p-0">{child}</td></tr></tbody>;
  }
  return child;
}
...
export function AdminTable({ children, minWidth = "min-w-[760px]" }: { children: ReactNode; minWidth?: string }) {
  ...
  return (
    <AdminCard className={`mt-4 overflow-hidden ${minWidth === "820px" ? "border-slate-200/80 shadow-sm" : ""}`}>
