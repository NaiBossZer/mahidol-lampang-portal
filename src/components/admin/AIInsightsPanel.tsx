...
export default function AIInsightsPanel() {
  ...
  return (
    <section className="rounded-xl border border-violet-200 bg-gradient-to-r from-violet-50 to-purple-50 p-4 shadow-xs">
      <div className="mb-3 flex items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          <Sparkles className="h-4 w-4 text-violet-600" />
          <h2 className="text-[14px] font-bold text-violet-900">AI Insights และสมรรถนะแบบสอบถาม</h2>
        </div>
        <span className="rounded-full bg-violet-100 px-2 py-1 text-[10px] font-medium text-violet-600">AI Studio Integration</span>
      </div>
      {loading ? (
...
function Metric({ label, value, suffix }: { label: string; value: string; suffix: string }) {
  return <div className="rounded-lg border border-violet-100 bg-white/60 p-3"><p className="text-[10px] font-medium text-violet-600">{label}</p><p className="text-lg font-bold text-violet-900">{value}</p><p className="text-[10px] text-slate-500">{suffix}</p></div>;
}
...
function Bar({ label, width, text }: { label: string; value: number; width: number; text: string }) {
  return <div className="flex items-center gap-2"><span className="w-12 shrink-0 text-[10px] text-slate-500">{label}: {text}</span><div className="h-2 min-w-0 flex-1 overflow-hidden rounded-full bg-slate-200"><div className="h-full rounded-full bg-violet-500" style={{ width: `${width}%` }} /></div></div>;
}
