...
export function LoadingState({ label = "กำลังโหลด..." }: { label?: string }) {
  return (
    <div
      className="flex min-h-32 items-center justify-center gap-2 rounded-2xl border border-border bg-white p-6 text-sm font-semibold text-muted-ink"
      role="status"
      aria-live="polite"
    >
...
export function EmptyState({
  ...
  return (
    <div className="flex min-h-40 flex-col items-center justify-center rounded-2xl border border-dashed border-border bg-white p-8 text-center">
      ...
      {description && (
        <p className="mt-1 max-w-md text-sm leading-6 text-muted-ink">{description}</p>
      )}
    </div>
  );
}
...
export function ErrorState({
  ...
  description = "ไม่สามารถแสดงข้อมูลได้ในขณะนี้",
}: {
  title?: string;
  description?: string;
}) {
