...
const navGroups: readonly NavGroup[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { to: "/admin/surveys", label: "จัดการแบบสอบถาม", icon: ClipboardList, permission: "survey.read" },
      { to: "/admin/surveys/analytics", label: "ผลการวิเคราะห์ Survey", icon: BarChart3, permission: "survey.audit.read" },
      { to: "/admin/activities/occurrences", label: "รอบการจัดกิจกรรม", icon: ClipboardList, permission: "activities.read" },
      { to: "/admin/activities/photos", label: "คลังภาพกิจกรรม", icon: FolderOpen, permission: "activities.read" },
    ],
  },
  {
    id: "content",
    ...
    icon: FolderOpen,
    items: [
      { to: "/admin/cms", label: "คลังเอกสารราชการ / CMS", icon: FolderOpen, permission: "cms.read" },
    ],
  },
  {
    id: "facility",
    ...
    icon: ShieldCheck,
    items: [
      { to: "/admin/facility-safety", label: "อาคารและความปลอดภัย", icon: ShieldCheck, permission: "facility.read" },
    ],
  },
  {
    id: "administration",
    ...
    items: [
      ...
      { to: "/admin/audit-trail", label: "ประวัติการใช้งาน", icon: FileText, permission: "system.read" },
...
export function AdminAppShell({ children }: { children: ReactNode }) {
  ...
  const [openGroups, setOpenGroups] = useState<Record<string, boolean>>({ "ai-studio": true, activities: true });
  ...
  const visibleGroups = useMemo(() => navGroups.map((group) => ({ ...group, items: group.items.filter((item) => hasPermission(item.permission)) })).filter((group) => group.items.length > 0), [role, permissions]);
  const isGroupActive = (group: NavGroup) => group.items.some((item) => isItemActive(location.pathname, item));
  const isGroupOpen = (group: NavGroup) => Boolean(openGroups[group.id] ?? isGroupActive(group));
  const toggleGroup = (groupId: string) => setOpenGroups((current) => ({ ...current, [groupId]: !(current[groupId] ?? false) }));
  ...
  return (
    <div className={cn("admin-app-shell min-h-screen font-sans text-slate-900", location.pathname === "/dashboard" ? "admin-dashboard-shell" : "admin-modern-shell")}>
      {mobileOpen && <div role="button" tabIndex={0} aria-label="ปิดเมนูนำทาง" className="fixed inset-0 z-40 bg-black/60 backdrop-blur-xs lg:hidden" onClick={() => setMobileOpen(false)} onKeyDown={(event) => { if (event.key === "Escape" || event.key === "Enter") setMobileOpen(false); }} />}
      <aside id="app-sidebar" className={cn("fixed inset-y-0 left-0 z-50 flex h-screen w-[var(--admin-sidebar-width)] shrink-0 flex-col justify-between select-none bg-[#0c2340] text-white shadow-xl transition-transform duration-300 ease-in-out", mobileOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0")}>
        <div className="flex min-h-0 flex-col">
          <div className="flex h-16 items-center justify-between border-b border-white/10 px-4">
            <Link to="/dashboard" onClick={() => setMobileOpen(false)} className="flex min-w-0 items-center gap-3 text-left"><div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-white shadow-sm"><span className="flex h-7 w-7 items-center justify-center rounded-full border-2 border-[#0c2340] text-xs font-bold tracking-tighter text-[#0c2340]">MU</span></div><div className="flex min-w-0 flex-col"><span className="truncate text-[14px] font-bold leading-tight tracking-tight text-white">Mahidol Lampang Portal</span><span className="mt-0.5 truncate text-[10px] font-normal leading-tight text-slate-300">ระบบบริหารจัดการและข้อมูลเชิงสถิติ</span></div></Link>
