...
const surveyGroup: NavGroup = { id: "surveys", label: "บริหารกิจกรรม & Survey", icon: Activity, items: [
...
export function AdminAppShell({ children }: { children: ReactNode }) {
  const location = useLocation(); const navigate = useNavigate(); const { role, permissions } = useAdminAuth(); const [mobileOpen, setMobileOpen] = useState(false); const [surveySubmenuOpen, setSurveySubmenuOpen] = useState(true);
  const hasPermission = (permission: AdminPermission) => role === "SUPER_ADMIN" || permissions.includes(permission);
  const visibleOverview = useMemo(() => hasPermission(overviewItem.permission), [role, permissions]); const visibleSurveyItems = useMemo(() => surveyGroup.items.filter((item) => hasPermission(item.permission)), [role, permissions]); const visibleNavItems = useMemo(() => navItems.filter((item) => hasPermission(item.permission)), [role, permissions]);
  ...
  const currentBreadcrumb = PATH_TITLE_MAP[location.pathname] ?? { group: "ระบบหลังบ้าน", title: "Central Admin" }; const roleText = (role && ROLE_DISPLAY_MAP[role]) ?? role ?? "Staff"; const surveyActive = visibleSurveyItems.some((item) => isItemActive(location.pathname, item)); const contentOffsetClass = location.pathname === "/dashboard" ? "lg:ml-[var(--admin-sidebar-width)]" : "";
  return <div className={cn("admin-app-shell min-h-screen font-sans text-slate-900", location.pathname === "/dashboard" ? "admin-dashboard-shell" : "admin-modern-shell")}>
    ...
    <aside id="app-sidebar" className={cn("fixed inset-y-0 left-0 z-50 flex h-screen w-[var(--admin-sidebar-width)] shrink-0 flex-col justify-between select-none bg-[#0c2340] text-white shadow-xl transition-transform duration-300 ease-in-out", mobileOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0")}>
      ...
      <nav className="max-h-[calc(100vh-140px)] space-y-1 overflow-y-auto p-3 text-[13px]" aria-label="Admin navigation">
        ...
        {visibleSurveyItems.length > 0 && <div className="space-y-1"><button type="button" onClick={() => setSurveySubmenuOpen((open) => !open)} className={cn("flex w-full items-center justify-between rounded-lg px-3 py-2 text-left transition-colors", surveyActive ? "bg-[#163a66] font-medium text-white shadow-xs" : "text-slate-300 hover:bg-white/5 hover:text-white")} aria-expanded={surveySubmenuOpen}><span className="flex min-w-0 items-center gap-3"><Activity className="h-[19px] w-[19px] shrink-0" /><span className="truncate">{surveyGroup.label}</span></span><ChevronDown className={cn("h-4 w-4 shrink-0 text-slate-400 transition-transform", surveySubmenuOpen && "rotate-180")} /></button>{surveySubmenuOpen && <div className="space-y-1 py-1 pl-9 pr-2 text-[12px]">{visibleSurveyItems.map((item) => { const ItemIcon = item.icon; const active = isItemActive(location.pathname, item); return <Link key={item.to} to={item.to} onClick={() => setMobileOpen(false)} className={cn("flex w-full items-center justify-between rounded px-2 py-1.5 text-left transition-colors", active ? "bg-white/10 font-semibold text-sky-300" : "text-slate-400 hover:bg-white/5 hover:text-white")}><span className="flex min-w-0 items-center gap-2"><ItemIcon className="h-3.5 w-3.5 shrink-0" /><span className="truncate">{item.label}</span></span>{item.badge && <span className="ml-2 shrink-0 rounded bg-purple-600 px-1 py-0.5 text-[9px] font-bold text-white">{item.badge}</span>}</Link>; })}</div>}</div>}
        {visibleNavItems.map((item) => { const ItemIcon = item.icon; const active = isItemActive(location.pathname, item); return <Link key={item.to} to={item.to} onClick={() => setMobileOpen(false)} className={cn("flex w-full items-center justify-between rounded-lg px-3 py-2 transition-colors", active ? "bg-[#163a66] font-medium text-white shadow-xs" : "text-slate-300 hover:bg-white/5 hover:text-white")}><span className="flex min-w-0 items-center gap-3"><ItemIcon className="h-[19px] w-[19px] shrink-0" /><span className="truncate">{item.label}</span></span>{item.badge && <span className="ml-2 shrink-0 rounded bg-rose-500 px-1.5 py-0.5 text-[10px] font-bold leading-none text-white">{item.badge}</span>}</Link>; })}
      </nav></div>
