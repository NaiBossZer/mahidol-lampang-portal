...
const PATH_TITLE_MAP: Record<string, { group: string; title: string }> = {
  ...
  "/admin/organizations": { group: "ผู้ใช้งาน & สิทธิ์การเข้าถึง", title: "ผู้ใช้งาน & สิทธิ์การเข้าถึง" },
  "/admin/governance": { group: "การแจ้งเตือน", title: "การแจ้งเตือน" },
};
...
export function AdminAppShell({ children }: { children: ReactNode }) {
  ...
  const visibleUtilityItems = useMemo(
    () => adminUtilityItems.filter((item) => hasPermission(item.permission)),
    [role, permissions],
  );
    ...
    >
      ...
      >
            ...
            {visibleUtilityItems.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  {item.badge && (
                    <span className="ml-2 shrink-0 rounded bg-rose-500 px-1.5 py-0.5 text-[10px] font-bold leading-none text-white">
                      {item.badge}
                    </span>
                  )}
                </Link>
        ...
        <div className="border-t border-white/10 p-3">
          <div className="mb-1 flex w-full items-center justify-between rounded-lg p-2 text-left">
