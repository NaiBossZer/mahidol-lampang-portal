...
import { Building2, CalendarRange, ClipboardList, FileText, LayoutDashboard, LogOut, Menu, Settings2, ShoppingBag, X, RefreshCw, Bot, History, ListTodo, Search, Bell, BookOpen } from "lucide-react";
...
export function AdminAppShell({ children }: { children: ReactNode }) {
  const location = useLocation(); const navigate = useNavigate(); const { role, hasPermission, refresh } = useAdminAuth(); const [mobileOpen, setMobileOpen] = useState(false);
  ...
  return <div className={cn("min-h-screen bg-surface-warm text-slate-900", isDashboard && "admin-dashboard-shell")}>
    <button type="button" aria-label="เปิดเมนู" className="fixed left-4 top-4 z-50 grid h-10 w-10 place-items-center rounded-xl border border-slate-200 bg-white text-brand-navy shadow-sm lg:hidden" onClick={() => setMobileOpen(true)}><Menu className="h-5 w-5" /></button>
    ...
    <aside className={cn("fixed inset-y-0 left-0 z-50 flex w-64 flex-col bg-brand-navy text-white transition-transform lg:translate-x-0", mobileOpen ? "translate-x-0" : "-translate-x-full")}>
      <div className="flex h-16 items-center justify-between border-b border-white/10 px-5"><div><p className="text-xs font-semibold uppercase tracking-[0.14em] text-white/60">Mahidol Lampang</p><p className="mt-0.5 text-base font-bold">Central Admin</p></div><button type="button" className="lg:hidden" onClick={() => setMobileOpen(false)} aria-label="ปิด"><X className="h-5 w-5" /></button></div>
      <nav className="flex-1 overflow-y-auto px-3 py-4" aria-label="Admin navigation"><p className="px-3 pb-2 text-[11px] font-semibold uppercase tracking-[0.14em] text-white/40">Operations</p>{navItems.map((item) => { const Icon = item.icon; const active = location.pathname === item.to || (item.to !== "/dashboard" && location.pathname.startsWith(`${item.to}/`)); return <Link key={item.to} to={item.to} onClick={() => setMobileOpen(false)} className={cn("mb-1 flex min-h-10 items-center gap-3 rounded-xl px-3 text-sm font-semibold transition", active ? "bg-white text-brand-navy" : "text-white/75 hover:bg-white/10 hover:text-white")}><Icon className="h-4 w-4" />{item.label}</Link>; })}<p className="px-3 pb-2 pt-5 text-[11px] font-semibold uppercase tracking-[0.14em] text-white/40">AI Workspace</p>{aiNavItems.map((item) => { const Icon = item.icon; const active = location.pathname === item.to || location.pathname.startsWith(`${item.to}/`); return <Link key={item.to} to={item.to} onClick={() => setMobileOpen(false)} className={cn("mb-1 flex min-h-10 items-center gap-3 rounded-xl px-3 text-sm font-semibold transition", active ? "bg-white text-brand-navy" : "text-white/75 hover:bg-white/10 hover:text-white")}><Icon className="h-4 w-4" />{item.label}</Link>; })}</nav>
    ...
    <div className="lg:pl-64"><header className="sticky top-0 z-30 border-b border-slate-200 bg-white/95 backdrop-blur"><div className="flex min-h-16 items-center justify-end gap-2 px-4 sm:px-6"><div className="flex items-center gap-2"><span className="hidden text-xs text-slate-500 md:inline">{role ?? ""}</span>{isDashboard && <button type="button" aria-label="รีเฟรช Dashboard" onClick={() => { void refresh(); window.dispatchEvent(new CustomEvent("dashboard:refresh")); }} className="inline-flex h-10 items-center gap-2 rounded-xl border border-slate-200 px-3 text-sm font-semibold text-brand-navy"><RefreshCw className="h-4 w-4" />รีเฟรช</button>}<button type="button" aria-label="ค้นหา" className="grid h-10 w-10 place-items-center rounded-xl border border-slate-200 text-slate-600"><Search className="h-4 w-4" /></button><button type="button" aria-label="การแจ้งเตือน" className="grid h-10 w-10 place-items-center rounded-xl border border-slate-200 text-slate-600"><Bell className="h-4 w-4" /></button><button type="button" onClick={() => void logout()} className="grid h-10 w-10 place-items-center rounded-xl border border-slate-200 text-slate-600" aria-label="ออกจากระบบ"><LogOut className="h-4 w-4" /></button></div></div></header><main id="admin-main" className="min-h-[calc(100vh-4rem)]">{children}</main></div>
  </div>;
}
