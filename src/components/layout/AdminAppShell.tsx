...
export function AdminAppShell({ children }: { children: ReactNode }) {
  ...
  function toggleGroup(id: string) {
    setOpenGroups((current) => ({
      ...
      [id]: !(
        ...
        isGroupActive(
          location.pathname,
          visibleGroups.find((group) => group.id === id) ?? coreGroup,
        )
      ),
    }));
  }
  ...
  return (
    <div
      className={cn(
        "min-h-screen bg-surface-warm text-slate-900",
    ...
    >
      <button
        type="button"
        aria-label="เปิดเมนู"
        className="fixed left-4 top-4 z-50 grid h-10 w-10 place-items-center rounded-xl border bg-white text-brand-navy shadow-sm lg:hidden"
      ...
      <aside
        className={cn(
          "fixed inset-y-0 left-0 z-50 flex w-64 flex-col bg-brand-navy text-white transition-transform lg:translate-x-0",
      ...
      >
        <div className="flex h-16 items-center justify-between border-b border-white/10 px-5">
          <div>
            ...
            <p className="mt-0.5 text-base font-bold">Central Admin</p>
          </div>
          <button
            type="button"
            className="lg:hidden"
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-4" aria-label="Admin navigation">
          ...
          <p className="px-3 pb-2 pt-4 text-[11px] font-semibold uppercase tracking-[.14em] text-white/40">
            Operations
          </p>
          {visibleActivity && (
            <Link
              ...
              className={cn(
                "mb-1 flex min-h-10 items-center gap-3 rounded-xl px-3 text-sm font-semibold",
                isItemActive(location.pathname, activityItem)
                  ? "bg-white text-brand-navy"
                  : "text-white/75 hover:bg-white/10 hover:text-white",
              )}
            >
            ...
            return (
              <div key={group.id} className="mb-1">
                <button
                  ...
                  className={cn(
                    "flex min-h-10 w-full items-center gap-3 rounded-xl px-3 text-left text-sm font-semibold",
                    active
                    ...
                    {group.items.map((item) => {
                      ...
                      return (
                        <Link
                          ...
                          onClick={() => setMobileOpen(false)}
                          className={cn(
                            "mb-1 flex min-h-9 items-center gap-3 rounded-lg px-3 text-[13px] font-medium",
                            itemActive
                              ? "bg-white text-brand-navy"
        ...
        <div className="border-t border-white/10 p-3">
          <button
            type="button"
            onClick={() => void logout()}
            className="flex min-h-10 w-full items-center gap-3 rounded-xl px-3 text-sm font-semibold text-white/75 hover:bg-white/10 hover:text-white"
          >
      ...
      <div className="lg:pl-64">
        <header className="sticky top-0 z-30 border-b border-slate-200 bg-white/95 backdrop-blur">
          <div className="flex min-h-16 items-center justify-end gap-2 px-4 sm:px-6">
            <div className="flex items-center gap-2">
              ...
              {isDashboard && (
                <button
                  ...
                  className="inline-flex h-10 items-center gap-2 rounded-xl border px-3 text-sm font-semibold text-brand-navy"
                >
