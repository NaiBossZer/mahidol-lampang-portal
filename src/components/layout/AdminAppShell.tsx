...
type NavItem = { to: string; label: string; icon: typeof Activity; permission: AdminPermission; badge?: string; external?: boolean };
...
export function AdminAppShell({ children }: { children: ReactNode }) {
  ...
  const [mobileOpen, setMobileOpen] = useState(false); const [sidebarCollapsed, setSidebarCollapsed] = useState(false); const [openGroups, setOpenGroups] = useState<Record<string, boolean>>({ "ai-studio": true, activities: true });
  ...
  const contentOffsetClass = location.pathname === "/dashboard" ? (sidebarCollapsed ? "" : "lg:ml-[var(--admin-sidebar-width)]") : "";
  return <div className={cn("admin-app-shell min-h-screen font-sans text-slate-900", location.pathname === "/dashboard" ? "admin-dashboard-shell" : "admin-modern-shell")}>
    ...
    <aside id="app-sidebar" className={cn("fixed inset-y-0 left-0 z-50 flex h-screen w-[var(--admin-sidebar-width)] shrink-0 flex-col justify-between select-none bg-[#0c2340] text-white shadow-xl transition-transform duration-300 ease-in-out", mobileOpen ? "translate-x-0" : sidebarCollapsed ? "-translate-x-full" : "-translate-x-full lg:translate-x-0")}>
      <div className="flex min-h-0 flex-col"><div className="flex h-16 items-center justify-between border-b border-white/10 px-4"><Link to="/dashboard" onClick={() => setMobileOpen(false)} className="flex min-w-0 items-center gap-3 text-left"><div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-white shadow-sm"><span className="flex h-7 w-7 items-center justify-center rounded-full border-2 border-[#0c2340] text-xs font-bold tracking-tighter text-[#0c2340]">MU</span></div><div className="flex min-w-0 flex-col"><span className="truncate text-[14px] font-bold leading-tight tracking-tight text-white">Mahidol Lampang Portal</span><span className="mt-0.5 truncate text-[10px] font-normal leading-tight text-slate-300">ระบบบริหารจัดการและข้อมูลเชิงสถิติ</span></div></Link><div className="flex items-center gap-1"><button type="button" onClick={() => setSidebarCollapsed(true)} className="hidden rounded-lg p-1.5 text-slate-400 hover:bg-white/10 hover:text-white lg:inline-flex" aria-label="ซ่อน Sidebar" title="ซ่อน Sidebar"><X className="h-4 w-4" /></button><button type="button" onClick={() => setMobileOpen(false)} className="p-1 text-slate-400 hover:text-white lg:hidden" aria-label="ปิดเมนู"><X className="h-5 w-5" /></button></div></div>
    ...
    <div className={cn("min-h-screen", contentOffsetClass)}><header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-slate-200 bg-white/95 px-4 backdrop-blur sm:px-6"><div className="flex items-center gap-3"><button type="button" className={cn("rounded-lg p-2 text-slate-600 hover:bg-slate-100", sidebarCollapsed ? "inline-flex" : "lg:hidden")} onClick={() => sidebarCollapsed ? setSidebarCollapsed(false) : setMobileOpen(true)} aria-label={sidebarCollapsed ? "แสดง Sidebar" : "เปิดเมนู"} title={sidebarCollapsed ? "แสดง Sidebar" : "เปิดเมนู"}><Menu className="h-5 w-5" /></button><div className="hidden items-center gap-2 text-xs text-slate-400 md:flex"><span>{currentBreadcrumb.group}</span><span>/</span><span className="font-semibold text-slate-700">{currentBreadcrumb.title}</span></div></div><div className="flex items-center gap-2"><button type="button" className="rounded-lg p-2 text-slate-500 hover:bg-slate-100" aria-label="ค้นหา"><Search className="h-4 w-4" /></button><button type="button" onClick={() => navigate("/admin/ai-studio-workspace")} className="hidden rounded-lg bg-[#002d62] px-3 py-2 text-xs font-bold text-white sm:inline-flex"><Sparkles className="mr-1.5 h-3.5 w-3.5" />AI Studio</button><button type="button" onClick={() => navigate("/admin/notifications")} className="relative rounded-lg p-2 text-slate-500 hover:bg-slate-100" aria-label="การแจ้งเตือน"><Bell className="h-4 w-4" /><span className="absolute right-1 top-1 h-1.5 w-1.5 rounded-full bg-rose-500" /></button></div></header><main>{children}</main></div>
  </div>;
}
