...
export function EmbeddedSystemView({ url, title, systemLabel }: EmbeddedSystemViewProps) {
  ...
  return (
    <div className="flex min-h-dvh w-full flex-col bg-white">
      <header
        className="border-b border-slate-200 bg-[#002D62] px-3 py-2 text-white sm:px-4"
        aria-label={`${systemLabel} navigation`}
      >
      ...
      <div className="relative min-h-0 flex-1" aria-busy={loading}>
        {loading && !failed && (
          ...
          >
            <Loader2
              className="mr-2 h-5 w-5 shrink-0 animate-spin text-[#F2A900]"
              aria-hidden="true"
            />
        ...
        {failed && (
          ...
          >
            ...
            <p className="mt-1 max-w-md text-sm text-slate-500">
              ระบบอาจปิดปรับปรุงหรือเครือข่ายตอบสนองช้า
            </p>
