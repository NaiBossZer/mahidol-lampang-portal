...
export function AICommandBar({
  ...
  return (
    <form
      onSubmit={submit}
      className={cn(
        "flex items-center gap-2 rounded-2xl border border-slate-200 bg-white shadow-sm",
        compact ? "p-1.5" : "p-2",
      )}
    >
      <div
        className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-brand-navy text-white"
        aria-hidden="true"
      >
      ...
      <button
        type="submit"
        className="inline-flex min-h-9 items-center gap-1.5 rounded-xl bg-brand-navy px-3 text-xs font-bold text-white transition hover:opacity-90 disabled:opacity-50"
        disabled={!value.trim()}
      >
