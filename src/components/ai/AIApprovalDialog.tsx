...
export type AIApprovalDialogProps = {
  open: boolean;
  title: string;
  summary: string;
  changes: string[];
      ...
      >
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            <h2 id="ai-approval-title" className="text-lg font-bold text-brand-navy">
              {title}
            </h2>
        ...
        <div className="mt-5 rounded-xl bg-slate-50 p-4">
          ...
          <ul className="mt-2 space-y-2">
            {changes.map((change) => (
              <li key={change} className="flex gap-2 text-sm text-slate-700">
        ...
        <div className="mt-5 flex justify-end gap-2">
          <button
            type="button"
            onClick={onReject}
            className="min-h-10 rounded-xl border border-slate-200 px-4 text-sm font-bold text-slate-700 hover:bg-slate-50"
          >
            ไม่อนุมัติ
          </button>
          <button
            type="button"
            onClick={onApprove}
            className="min-h-10 rounded-xl bg-brand-navy px-4 text-sm font-bold text-white hover:opacity-90"
          >
            อนุมัติและดำเนินการ
          </button>
        </div>
