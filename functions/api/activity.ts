...
export async function onRequestGet({ request, env }: { request: Request; env: Record<string, unknown> }) {
  ...
  try {
    const rows = await supabaseRest<Record<string, unknown>[]>(env, "activities", { select: SELECT, slug: `eq.${slug}`, status: "eq.published", limit: "1" });
    const activity = rows[0];
    if (!activity) return json({ error: "Activity not found" }, 404);
    const activityId = String(activity.id);
    const [photos, outcomes] = await Promise.all([
      supabaseRest<Record<string, unknown>[]>(env, "activity_photos", { select: "id,activity_id,image_url,thumbnail_url,caption,alt_text,sort_order,is_cover,created_at", activity_id: `eq.${activityId}`, order: "sort_order.asc" }),
      supabaseRest<Record<string, unknown>[]>(env, "activity_outcomes", { select: "id,activity_id,metric_name,metric_value,unit,description,created_at", activity_id: `eq.${activityId}` }),
    ]);
