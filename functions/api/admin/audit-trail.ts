...
async function sb(env: Env, token: string, path: string) {
...
export async function onRequest({ request, env }: { request: Request; env: Env }) {
  ...
  try {
    ...
    if (table) params.set("table_name", `eq.${table}`);
    if (action) params.set("action", `eq.${action}`);
    return json({ success: true, data: await sb(env, token, `audit_logs?${params.toString()}`) });
  } catch (error) {
    console.error("/api/admin/audit-trail", error);
    return json(
      {
        success: false,
        error: error instanceof Error ? error.message : "ไม่สามารถโหลด Audit Trail ได้",
      },
      500,
    );
  }
