...
function canPermission(role: AdminRole, permission: string) { return permissionsForRole(role).includes(permission as never); }
...
function contractField(entityType: string, fieldKey: string) { return Boolean(MEDIA_CONTRACTS[entityType]?.[fieldKey]); }
...
export async function onRequest({ request, env }: { request: Request; env: Env }) {
  try {
    ...
    if (request.method === "POST") {
      ...
      if (!contractField(entityType, fieldKey)) return json({ success: false, error: "Media field นี้ยังไม่มี Storage Contract" }, 400);
      if (!canWrite(role, entityType, true)) return json({ success: false, error: "Forbidden" }, 403);
    ...
    if (request.method === "DELETE") {
      ...
      const rows = await rest(env, token, `portal_media_assets?id=eq.${id}&select=id,storage_path,entity_type,field_key`) as MediaRow[];
      const row = rows[0];
      if (!row) return json({ success: false, error: "ไม่พบไฟล์" }, 404);
      const entityType = String(row.entity_type ?? ""); const fieldKey = String(row.field_key ?? "");
      if (!contractField(entityType, fieldKey) || !canWrite(role, entityType, false)) return json({ success: false, error: "Forbidden" }, 403);
