...
export async function authorize(request: Request, env: Env, permission: string) {
  ...
  if (!permissionsForRole(role as AdminRole).includes(permission as never))
...
export function methodNotAllowed(methods: string[]) {
  return json({ success: false, error: "Method Not Allowed" }, 405, { Allow: methods.join(", ") });
}
export function idFromUrl(request: Request) {
  return new URL(request.url).searchParams.get("id");
}
export function clean(value: unknown, max = 10000) {
  const text = value == null ? "" : String(value).trim();
  return text.slice(0, max);
}
export function nullable(value: unknown, max = 10000) {
