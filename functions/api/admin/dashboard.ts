...
export async function onRequestGet({ request, env }: { request: Request; env: Env }) {
  ...
  try {
    const [activities, occurrences, centers, organizations, surveys, responses] = await Promise.all([
      supabaseUserRest(env, accessToken, "activities", {
      ...
      supabaseUserRest(env, accessToken, "activity_occurrences", {
        select: "id,activity_id,occurrence_no,start_at,end_at,status,cancellation_reason,participant_count,location_type,location_detail",
        order: "start_at.desc",
      }),
