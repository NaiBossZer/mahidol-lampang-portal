  ...
  try {
    const [
      ...
      supabaseUserRest<DashboardActivityRow[]>(env, accessToken, "activities", {
        select: "id,title,activity_date,category,status,participant_count,participants,summary,content,objective,key_activities,outcomes,impact,featured_image",
        order: "activity_date.desc",
      }),
      supabaseUserRest(env, accessToken, "activity_occurrences", {
        select:
          "id,activity_id,occurrence_no,start_at,end_at,status,cancellation_reason,participant_count,location_type,location_detail",
        order: "start_at.desc",
      }),
      supabaseUserRest(env, accessToken, "learning_centers", {
        select: "id,name,slug,type,status",
        status: "eq.active",
        order: "name.asc",
      }),
      supabaseUserRest(env, accessToken, "organizations", {
        select: "id,name,organization_type,parent_organization_id,status,display_order",
        status: "eq.active",
        order: "display_order.asc,name.asc",
      }),
      supabaseUserRest(env, accessToken, "occurrence_surveys", {
        select: "id,occurrence_id,title,enabled,anonymous,open_at,close_at,welcome_text",
      }),
      supabaseUserRest(env, accessToken, "survey_questions", {
        select: "id,survey_id,section_key,question_type,question_text,order_index,active,scale_min,scale_max",
        active: "eq.true",
        order: "survey_id.asc,order_index.asc",
      }),
      supabaseUserRest(env, accessToken, "survey_answers", {
        select: "id,response_id,question_id,answer_number,answer_text,answer_options",
      ...
      supabaseUserRest(env, accessToken, "activity_learning_centers", {
        select: "activity_id,learning_center_id",
      }),
      supabaseUserRest(env, accessToken, "activity_organizers", {
        select: "activity_id,organization_id,organizer_role",
      }),
      supabaseUserRest<DashboardMediaRow[]>(env, accessToken, "activity_media", {
        select:
          "id,activity_id,occurrence_id,public_url,media_type,caption,is_post_event,display_order,status,created_at",
