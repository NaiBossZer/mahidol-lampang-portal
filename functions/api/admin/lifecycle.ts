...
export async function onRequest({ request, env }: { request: Request; env: Env }) {
  try {
    ...
    const r = await fetch(`${url}/rest/v1/${table}?id=eq.${encodeURIComponent(id)}`, {
      ...
      body: JSON.stringify(
        type === "learning_center"
          ? { lifecycle_status: status, updated_at: new Date().toISOString() }
    ...
    console.error("/api/admin/lifecycle", e);
    return json(
      { success: false, error: e instanceof Error ? e.message : "Lifecycle failed" },
      500,
    );
  }
}
