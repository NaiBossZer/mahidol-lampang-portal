...
export async function onRequest({ request, env }: { request: Request; env: Env }) {
  try {
    ...
    if (request.method === "GET")
      return json({
        ...
        data: await call(
          env,
          token,
          "ai_tools?select=id,tool_key,name,description,domain,endpoint,method,risk_level,permission,enabled&order=domain.asc,tool_key.asc",
        ),
      });
    if (request.method !== "POST")
      return json({ success: false, error: "Method Not Allowed" }, 405, { Allow: "GET, POST" });
    const body = (await request.json()) as { toolKey?: string; input?: Record<string, unknown> };
    if (!body.toolKey) return json({ success: false, error: "toolKey required" }, 400);
    const tools = await call(
      env,
      token,
      `ai_tools?tool_key=eq.${encodeURIComponent(body.toolKey)}&enabled=eq.true&limit=1`,
    );
    const tool = tools?.[0];
    ...
    const execution = Array.isArray(exec) ? exec[0] : exec;
    if (needsApproval) {
      await call(env, token, "admin_notifications", {
        method: "POST",
        ...
        body: JSON.stringify({
          ...
          body: `${tool.name} · risk ${tool.risk_level}`,
          link: `/admin/ai/approval?executionId=${execution.id}`,
        }),
      });
      return json({ success: true, requiresApproval: true, data: execution }, 202);
    }
    ...
    const response = await fetch(target, {
      ...
      body: method === "GET" ? undefined : JSON.stringify(body.input ?? {}),
    });
    const result = await response.json().catch(() => null);
    await call(env, token, `ai_executions?id=eq.${execution.id}`, {
      method: "PATCH",
    ...
    return json(
      { success: response.ok, data: result, executionId: execution.id },
      response.ok ? 200 : response.status,
    );
  } catch (e) {
    console.error("/api/admin/ai-tools", e);
    return json(
      { success: false, error: e instanceof Error ? e.message : "AI manager failed" },
      500,
    );
  }
}
