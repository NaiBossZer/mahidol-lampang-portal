...
function token(r: Request) {
  const p = (r.headers.get("Cookie") ?? "")
    .split(";")
    .map((x) => x.trim())
    .find((x) => x.startsWith("sb_access_token="));
  return p ? decodeURIComponent(p.slice("sb_access_token=".length)) : null;
}
async function sb(env: Env, t: string, path: string, init: RequestInit = {}) {
...
export async function onRequest({ request, env }: { request: Request; env: Env }) {
  ...
  try {
    if (request.method === "GET")
      return json({
        success: true,
        data: await sb(env, t, "organizations?select=*&order=name.asc"),
      });
    ...
    if (request.method === "PUT") {
      ...
      return json({
        success: true,
        data: (
          await sb(env, t, `organizations?id=eq.${encodeURIComponent(id)}`, {
            method: "PATCH",
            headers: { Prefer: "return=representation" },
            body: JSON.stringify(b),
          })
    ...
    console.error("/api/admin/organizations", e);
    return json(
      { success: false, error: e instanceof Error ? e.message : "ไม่สามารถจัดการองค์กรได้" },
      500,
    );
  }
}
