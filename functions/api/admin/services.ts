...
function values(body: Record<string, unknown>) {
  const title = clean(body.title, 255); const slug = clean(body.slug, 255); const linkType = clean(body.linkType, 30) || "INTERNAL"; const status = clean(body.status, 30) || "draft";
  if (!title || !slug || !["INTERNAL", "EXTERNAL", "CONTACT"].includes(linkType) || !["draft", "published", "archived"].includes(status)) return null;
  return { title, slug, summary: nullable(body.summary), description: nullable(body.description), icon: nullable(body.icon, 100), featured_image: nullable(body.featuredImage), link_type: linkType, link_url: nullable(body.linkUrl), sort_order: Number.isInteger(Number(body.sortOrder)) ? Number(body.sortOrder) : 0, status, published_at: status === "published" ? new Date().toISOString() : null, updated_at: new Date().toISOString() };
}
export async function onRequest({ request, env }: { request: Request; env: Env }) {
  const permission = request.method === "GET" ? "services.read" : request.method === "POST" ? "services.create" : request.method === "DELETE" ? "services.archive" : "services.update";
