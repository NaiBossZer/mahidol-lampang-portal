...
export async function onRequest({ request, env }: { request: Request; env: Env }) {
  try {
    ...
    if (request.method === "GET") {
      ...
      const questions = await sb<Row[]>(env, `survey_questions?survey_id=eq.${encodeURIComponent(surveyId)}&active=eq.true&select=id,section_key,question_text,question_type,required,order_index,options,scale_min,scale_max&order=order_index.asc`);
    ...
    const body = await request.json() as { surveyId?: string; activityId?: string; occurrenceId?: string; ageGroup?: string; affiliation?: string; everJoined?: string; channels?: string; feedback?: string; pdpaConsent?: boolean; answers?: Answer[] };
    if (!body.surveyId || body.pdpaConsent !== true) return json({ success: false, error: "ต้องระบุ surveyId และยินยอม PDPA" }, 400);
    const surveys = await sb<Row[]>(env, `occurrence_surveys?id=eq.${encodeURIComponent(body.surveyId)}&enabled=eq.true&select=id,occurrence_id,open_at,close_at`);
    const survey = surveys[0];
    if (!survey) return json({ success: false, error: "แบบสอบถามไม่พร้อมรับคำตอบ" }, 409);
    const now = Date.now();
    if (survey.open_at && now < Date.parse(String(survey.open_at))) return json({ success: false, error: "แบบสอบถามยังไม่เปิด" }, 409);
    if (survey.close_at && now > Date.parse(String(survey.close_at))) return json({ success: false, error: "แบบสอบถามปิดรับคำตอบแล้ว" }, 409);
    const occurrenceId = body.occurrenceId || String(survey.occurrence_id);
    const occurrences = await sb<Row[]>(env, `activity_occurrences?id=eq.${encodeURIComponent(occurrenceId)}&status=neq.cancelled&status=neq.archived&select=id,activity_id`);
    ...
    const responseRows = await sb<Row[]>(env, "survey_responses", { method: "POST", headers: { Prefer: "return=representation" }, body: JSON.stringify({ activity_id: activityId, occurrence_id: occurrenceId, survey_id: body.surveyId, age_group: body.ageGroup ?? "", affiliation: body.affiliation ?? "", ever_joined: body.everJoined ?? "", channels: body.channels ?? "", feedback: body.feedback ?? "", pdpa_consent: true }) });
    ...
    return json({ success: false, error: error instanceof Error ? error.message : "ไม่สามารถส่งแบบสอบถามได้" }, 500);
  }
}
