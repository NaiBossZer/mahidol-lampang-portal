...
export async function onRequestGet({ env }: { env: Env }) {
  ...
  try {
    ...
    const media = rows.length
      ? await supabaseRest<MediaRow[]>(env, "activity_media", {
          select: "activity_id,public_url,display_order,created_at",
          activity_id: `in.(${rows.map((row) => String(row.id)).join(",")})`,
          status: "eq.active",
    ...
    for (const item of media) {
      const current = grouped.get(item.activity_id) ?? [];
      current.push(item);
      grouped.set(item.activity_id, current);
    }
    return json({
      success: true,
      data: rows.map((row) => toActivity(row, grouped.get(String(row.id)) ?? [])),
    });
  } catch (error) {
