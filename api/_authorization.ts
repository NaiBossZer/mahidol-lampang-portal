...
export function adminIdentity(req: ApiRequest): AdminIdentity | null {
  const raw = accessToken(req); if (!raw) return null;
  const [headerPart, payloadPart, signature] = raw.split("."); if (!headerPart || !payloadPart || !signature) return null;
  const header = decodePart(headerPart); const payload = decodePart(payloadPart);
  ...
  const expectedBytes = Buffer.from(expected); const actualBytes = Buffer.from(signature);
  ...
  return isAdminRole(roleValue)
    ? { id: payload.sub, email: typeof payload.email === "string" ? payload.email : null, role: roleValue }
    : null;
}

export function requireAdmin(req: ApiRequest, res: ApiResponse): AdminIdentity | null {
  const identity = adminIdentity(req);
  if (!identity) { json(res, 401, { error: "Unauthorized" }); return null; }
  return identity;
}

export function requirePermission(req: ApiRequest, res: ApiResponse, permission: string): AdminIdentity | null {
  const identity = requireAdmin(req, res);
  if (!identity) return null;
  if (!hasAdminPermission(identity.role, permission)) { json(res, 403, { error: "Forbidden" }); return null; }
  return identity;
}
