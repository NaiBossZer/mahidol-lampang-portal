...
export default async function handler(req: ApiRequest, res: ApiResponse) {
  ...
  if (!role) return json(res, 403, { success: false, data: { authenticated: true, authorized: false } });
  return json(res, 200, {
    success: true,
    data: { authenticated: true, authorized: true, user: { id: user.id, email: user.email ?? null }, role, permissions: permissionsForRole(role) },
  });
}
