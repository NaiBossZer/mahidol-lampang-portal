...
export default async function handler(req: ApiRequest, res: ApiResponse) {
  ...
  try {
    ...
    if (req.method === "GET")
      return json(res, 200, {
        success: true,
        data: await db.select().from(partners).orderBy(asc(partners.name)),
      });
    const body = await parseObject(req);
    if (!body || !requiredString(body.name))
      return json(res, 400, { error: "ข้อมูลภาคีเครือข่ายไม่ถูกต้อง" });
    const values = {
      name: requiredString(body.name),
      type: stringValue(body.type, 100),
      logo: stringValue(body.logo),
    ...
    if ((req.method === "PUT" || req.method === "PATCH") && id) {
      ...
      if (!row) return json(res, 404, { error: "ไม่พบภาคีเครือข่าย" });
      return json(res, 200, { success: true, data: row });
    }
    return respondMethodNotAllowed(res, ["GET", "POST", "PUT", "PATCH"]);
  } catch (error) {
    console.error(error);
    return json(res, 500, { error: "ไม่สามารถจัดการภาคีเครือข่ายได้" });
  }
}
