...
function buildValues(body: Record<string, unknown> | null, existingPublishedAt?: Date | null) {
  ...
  return {
    ...
    icon: stringValue(body?.icon, 100),
    featuredImage: stringValue(body?.featuredImage),
    linkType,
    linkUrl: stringValue(body?.linkUrl),
    sortOrder,
    status,
...
export default async function handler(req: ApiRequest, res: ApiResponse) {
  ...
  try {
    ...
    if (req.method === "POST") {
      ...
      if (!values) return json(res, 400, { error: "ข้อมูลบริการไม่ถูกต้อง" });
      const [row] = await db.insert(services).values(values).returning();
      return json(res, 201, { success: true, data: row });
    }
    if ((req.method === "PUT" || req.method === "PATCH") && id) {
      const current = await db
        .select({ publishedAt: services.publishedAt })
        .from(services)
        .where(eq(services.id, id));
      const values = buildValues(body, current[0]?.publishedAt);
      if (!values) return json(res, 400, { error: "ข้อมูลบริการไม่ถูกต้อง" });
    ...
    if (req.method === "DELETE" && id) {
      const [row] = await db
        ...
        .where(eq(services.id, id))
        .returning({ id: services.id, status: services.status });
      if (!row) return json(res, 404, { error: "ไม่พบบริการ" });
      return json(res, 200, { success: true, data: row });
    }
