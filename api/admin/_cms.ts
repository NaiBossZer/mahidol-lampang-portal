...
export function requireAdmin(req: ApiRequest, res: ApiResponse) { return Boolean(requirePermission(req, res, "cms.read")); }
...
export function statusValue(value: unknown, fallback: CmsStatus = "draft") { const status = String(value ?? fallback); return CMS_STATUSES.includes(status as CmsStatus) ? status as CmsStatus : null; }
export function linkTypeValue(value: unknown, fallback: CmsLinkType = "INTERNAL") { const type = String(value ?? fallback); return CMS_LINK_TYPES.includes(type as CmsLinkType) ? type as CmsLinkType : null; }
export function respondMethodNotAllowed(res: ApiResponse, methods: string[]) { return methodNotAllowed(res, methods); }
