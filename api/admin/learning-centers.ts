...
export default async function handler(req: ApiRequest, res: ApiResponse) {
  ...
  try {
    ...
    if (req.method === "GET")
      return json(res, 200, {
        success: true,
        data: await db.select().from(learningCenters).orderBy(asc(learningCenters.name)),
      });
    ...
    if (
      ...
      return json(res, 400, { error: "ข้อมูลศูนย์การเรียนรู้ไม่ถูกต้อง" });
    const values = {
      name: requiredString(body.name),
      slug: requiredString(body.slug),
      type: type as (typeof types)[number],
      description: stringValue(body.description),
      province: stringValue(body.province, 100),
      district: stringValue(body.district, 100),
      subdistrict: stringValue(body.subdistrict, 100),
    ...
    if (req.method === "POST") {
      const [row] = await db.insert(learningCenters).values(values).returning();
      return json(res, 201, { success: true, data: row });
    }
    const id = idFromRequest(req);
    if ((req.method === "PUT" || req.method === "PATCH") && id) {
      const [row] = await db
        .update(learningCenters)
        .set(values)
        .where(eq(learningCenters.id, id))
        .returning();
      if (!row) return json(res, 404, { error: "ไม่พบศูนย์การเรียนรู้" });
      return json(res, 200, { success: true, data: row });
    }
    return respondMethodNotAllowed(res, ["GET", "POST", "PUT", "PATCH"]);
  } catch (error) {
    console.error(error);
    return json(res, 500, { error: "ไม่สามารถจัดการศูนย์การเรียนรู้ได้" });
  }
}
