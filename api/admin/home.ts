...
export default async function handler(req: ApiRequest, res: ApiResponse) {
  ...
  try {
    ...
    if (req.method === "GET")
      return json(res, 200, {
        success: true,
        data: await db.select().from(homeSections).orderBy(asc(homeSections.sortOrder)),
      });
    ...
    if (req.method === "POST") {
      ...
      if (
        !HOME_SECTION_KEYS.includes(sectionKey as (typeof HOME_SECTION_KEYS)[number]) ||
        sortOrder === null
      )
        return json(res, 400, { error: "ข้อมูลส่วนหน้าแรกไม่ถูกต้อง" });
      const [row] = await db
        .insert(homeSections)
        .values({
          sectionKey,
          title: stringValue(body?.title, 255),
          subtitle: stringValue(body?.subtitle, 500),
