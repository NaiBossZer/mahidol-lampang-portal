...
## Database
Migration ordering is canonical:
...
The `0002` collision between canonical CMS and LAC satisfaction has been removed. `admin_users` stores the approved identity/profile mapping and `admin_audit_log` stores server-side audit events; Supabase Auth remains the identity/RBAC authority.
