...
- `activity_photos` — normalized activity photo/gallery records; images are stored as URLs.
- `partners` + `activity_partners` — many-to-many partner organizations.
- `activity_outcomes` — measurable outcome/impact metrics.

Migration: `drizzle/0001_social_engagement.sql`.

D3-B does **not** apply the migration to production because the current workstation does not have a configured `DATABASE_URL`. Apply the SQL only after the production Neon connection is configured and backed up.
